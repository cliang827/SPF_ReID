...
if ~feedback_info_for_pcm14.consider_groundtruth_flag_for_pcm14(repeat_times) && ...
        ctrl_para.include_groundtruth_in_the_first_page_flag && ...
        curr_groundtruth_rank<=ctrl_para.image_num_per_page

    rand_feedback_ix_for_pcm14 = feedback_info_for_pcm14.rand_feedback_ix_for_pcm14;
    [~,tot_query_times, ~] = size(rand_feedback_ix_for_pcm14);
    [gallery_name_tab, ~] = feedback_stat(feedback_info_for_pcm14);
    ...
    feedback_info_for_pcm14.consider_groundtruth_flag_for_pcm14(repeat_times) = 1;
    feedback_info_for_pcm14.rand_feedback_ix_for_pcm14 = rand_feedback_ix_for_pcm14;
    setappdata(proc_handle, 'feedback_info_for_pcm14', feedback_info_for_pcm14);
end
    
% step 2: change index in feedback_info_for_pcm14 to that of prbgal_name_tab
prbgal_name_tab = ctrl_para.prbgal_name_tab;
feedback_set_temp = feedback_info_for_pcm14.rand_feedback_ix_for_pcm14{repeat_times, query_times};
[~, part_feedback_pair_num, ~] = size(feedback_set_temp);
for i=1:2
    for j = 1:part_feedback_pair_num
        for k=1:2
             if feedback_set_temp(i,j,k)>0
                gallery_name = feedback_info_for_pcm14.feedback_details{feedback_set_temp(i,j,k)}.gallery_name;
