...
ctrl_para.feedback_agent = 'mmap';  % {'mmap', 'mm15', 'mmap - 20171116 - backup'};
ctrl_para.treat_groundtruth_in_feedback_info_method = 'let-it-be'; % how to treat groundtruth when loading feedback_info
ctrl_para.include_groundtruth_in_the_first_page_flag = false;      % whether label groundtruth appearing in the 1st page
ctrl_para.follow_pcm14_feedback_protocol_flag = true;
ctrl_para.feedback_type = 'nv';                         % {'v', 'nv'};
ctrl_para.fbppr = [0 4];                                % feedback pair per run (in nv mode, fbppr<=7)
...
%                     if ix_info_tab(1)==2 && ix_info_tab(2)==2 && ix_info_tab(3)==1
%                         stop = 1;
%                     end
                    
                    [f_bat_for_pcm14{query_times, j}, feedback_set_for_pcm14{query_times, j}] = ...
                        test_baseline_pcm14(ctrl_para, feedback_info_tab{start+j}, groundtruth_feedback{start+j}, ix_info_tab);
        ...
        save(sprintf('.\\temp\\%s\\exp_report_%d.mat', exp_name_str, repeat_times), ...
            ...
            'baseline_cmc_result', 'cmc_result_for_pcm14', 'auc_result_for_pcm14');
    end
end
