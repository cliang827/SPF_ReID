...
for i=1:dataset_size
    rand('seed', i); %#ok<RAND> %keep multiple-iteration results contain single iteration result
    ...
    % step 2: load feedback info and check it's correctness
...
%                 box_conf(k)=1;
        ...
        % step 2.1: check feedback details' validity
        if sum(fbppr)>0
    ...
    % step 2.2: check feedback details' coverage
    if strcmp(feedback_type, 'v') && sum(fbppr)>0
        exp_times = size(exp_para_set, 1);
        for j=1:exp_times
            switch ctrl_para.dev_env
                case 'linux'
                    V_result_dir = ['./temp/' strrep(exp_name_set{j}, 'v', 'nv') '/details/'];
                case 'windows'
    ...
    % step 2.3: check feedback details' repeatness
    ...
    switch rand_feedback_ix_for_pcm14_method
        case 'allow-one-node-repreatness'
            % one node is allowed to appear in one pos/neg pair for many
            % times, e.g., following case is allowed: a-b, a-c, ... 
            
            for repeat_times=1:tot_repeat_times
                temp_ix = zeros(2, tot_fbppp_num, 2);      % first 2 : pos/neg
                                                           % second 2: torso/leg
                for k=1:2
                    ...
                    [pair_pos_ix, pair_neg_ix] = ind2sub([valid_pos_num, valid_neg_num], part_feedback_pair_ix(1:tot_fbppp_num));
                    temp_ix(:,:,k) = [valid_ix_pos(pair_pos_ix); valid_ix_neg(pair_neg_ix)];
                end

                for query_times=1:tot_query_times
                    feedback_pair_num = sum(fbppp(1:query_times));
                    if 1 == query_times
                        assert(0 == feedback_pair_num);
                        rand_feedback_ix_for_pcm14{repeat_times,query_times} = [];
                        continue;
                    end
                    rand_feedback_ix_for_pcm14{repeat_times,query_times} = temp_ix(:,1:feedback_pair_num,:);
                end
                
            end

        case 'forbid-node-repeatness'
            % one node is allowed to appear in one pos/neg pair at most
            % e.g., following case is forbidden: a-b, a-c, ... because a
            % appears two times in the pos/neg pair
            ...
            for repeat_times = 1:tot_repeat_times
                ...
                for k=1:2
                    ...
                    temp_ix(:,:,k) = [pos_ix(1:tot_fbppp_num); neg_ix(1:tot_fbppp_num)];
                end
            ...
            if strcmp('forbid-node-repeatness', rand_feedback_ix_for_pcm14_method) && ...
                ...
                for repeat_times=1:tot_repeat_times
                    for k=1:2
                        ...
                        new_valid_ix = valid_ix(randperm(valid_ix_num));
                        for query_times=1:tot_query_times
...
if err_num>1
    switch ctrl_para.dev_env
        case 'linux'
            save('./temp/feedback_invalid_list.mat', 'err_report');
            error('invalid feedback found! see err_report in ./temp/feedback_invalid_list.mat for details. \n');
        case 'windows'
